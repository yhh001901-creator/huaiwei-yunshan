<template>
  <div class="min-h-screen flex items-center justify-center font-sans relative overflow-hidden">
    <!-- 动态背景层 -->
    <div class="absolute inset-0 bg-gradient-to-br from-teal-50 via-emerald-50 to-cyan-50"></div>
    
    <!-- 装饰性圆形图案 -->
    <div class="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-teal-200/30 to-emerald-200/30 rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl animate-float-slow"></div>
    <div class="absolute bottom-0 right-0 w-[500px] h-[500px] bg-gradient-to-tl from-emerald-200/40 to-cyan-200/40 rounded-full translate-x-1/3 translate-y-1/3 blur-3xl animate-float-medium"></div>
    <div class="absolute top-1/4 right-1/4 w-64 h-64 bg-gradient-to-br from-cyan-200/25 to-teal-200/25 rounded-full blur-2xl animate-float-fast"></div>
    <div class="absolute bottom-1/3 left-1/4 w-48 h-48 bg-gradient-to-tr from-emerald-200/20 to-teal-200/20 rounded-full blur-2xl animate-float-medium"></div>
    ...
    <!-- 主容器 - 铺满整个屏幕 -->
    <div class="w-full h-full min-h-screen bg-transparent p-8 md:p-12 lg:p-20 relative z-10">
      <!-- 顶部装饰 -->
...
@keyframes float-medium {
  ...
  50% {
    transform: translate(38%, 28%) scale(1.05);
  }
}
